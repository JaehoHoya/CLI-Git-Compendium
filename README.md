...
** gabia에 등록된 네임 서버 4개를 AWS Route 53에 등록된 서버 주소로 변경하기 **
AWS의 Route 53에 등록된 4개의 네임서버 이름을 모두 복사한다 ns-581.awsdns-08.net. ns-250.awsdns-31.com. ns-1936.awsdns-50.co.uk. ns-1150.awsdns-15.org. gabia로그인 > MyGabia > 서비스 관리 > 해당 도메인 선택(체크) > 우측의 관리버튼 > 네임서버 설정 > Route 53에서 복사한 4개의 주소를 모두 입력 > 소유자 인증 > 적용

** 참고 **
네임서버 변경 사항이 전파되기까지 최대 48시간이 소요될 수 있음.
보통 10~20분 정도 경과 후 적용되는 것을 확인했음.
이후 도메인을 입력하여 AWS에서 호스팅하는 서비스에 정상적으로 연결되는지 확인
```
